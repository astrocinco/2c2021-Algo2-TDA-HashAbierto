...
const double FACTOR_CARGA_MAX = 3;
...
hash_t *hash_crear(hash_destruir_dato_t destruir_dato){
    ...
    void** arreglo_dinamico = malloc(sizeof(void*) * CAP_INICIAL);
...
bool hash_redimensionar2(hash_t* hash, size_t nueva_capacidad){ 
    void** nuevo_arreglo = malloc(sizeof(void*) *nueva_capacidad);
    ...
    for (size_t i = 0; i < vieja_cap; i++){
        if (!lista_esta_vacia(hash->arreglo[i])){
            lista_iter_t* iter = lista_iter_crear(hash->arreglo[i]);

            for (size_t j = 0; j < lista_largo(hash->arreglo[i]); j++){
                campo_t* actual = lista_iter_ver_actual(iter);
                size_t pos_nueva = djb2(actual->clave)%hash->capacidad;
                campo_t* nuevo = campo_crear(actual->clave, actual->dato);
                lista_insertar_ultimo(nuevo_arreglo[pos_nueva],nuevo);
                lista_iter_avanzar(iter);
            }//se puede hacer con lista_iterar
...
void *hash_borrar(hash_t *hash, const char *clave){
    ...
    campo_t* campo = borrar_campo(hash, clave);
    hash->carga--;
